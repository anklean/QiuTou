...
PyObject*  DocumentObjectPy::lock(PyObject * args)
{
	if (!PyArg_ParseTuple(args, ""))     // convert args: Python->C 
		return NULL;                    // NULL triggers exception 
	getDocumentObjectPtr()->setStatus(App::Restore , true);
	Py_Return;
}
PyObject*  DocumentObjectPy::unlock(PyObject * args)
{
	if (!PyArg_ParseTuple(args, ""))     // convert args: Python->C 
		return NULL;                    // NULL triggers exception 
	getDocumentObjectPtr()->setStatus(App::Restore, false);
	Py_Return;
}
PyObject*  DocumentObjectPy::recompute(PyObject * args)
{
	if (!PyArg_ParseTuple(args, ""))     // convert args: Python->C 
		return NULL;                    // NULL triggers exception 
	getDocumentObjectPtr()->recompute();
	Py_Return;
}
