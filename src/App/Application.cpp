...
void Application::initApplication(void)
{
	...
	{
		Console().Error("FreeCAD Init Failed", e.getStackTrace());
		throw e;
	}
	catch (Base::Exception& e)
	{
		Console().Error("FreeCAD Init Failed", e.what());
		throw e;
	}
}
...
void Application::LoadParameters(void)
{
    ...
    // Init parameter sets ===========================================================
    //默认取BinPath下面的配置文件
	mConfig["UserParameter"] = mConfig["BinPath"] + "user.cfg";
	Base::FileInfo file1(mConfig["UserParameter"]);
	if (file1.exists() == false) {
		mConfig["UserParameter"] = mConfig["AppHomePath"] + "user.cfg";
	}
	
	mConfig["SystemParameter"] = mConfig["BinPath"] + "system.cfg";
	Base::FileInfo file2(mConfig["SystemParameter"]);
	if (file2.exists() == false) {
		mConfig["SystemParameter"] = mConfig["AppHomePath"] + "system.cfg";
	}
